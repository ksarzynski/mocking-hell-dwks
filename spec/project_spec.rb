...
RSpec.describe 'Project' do
  context '#new' do
    let(:id) { 1 }
    let(:name) { 'projekt' }
    let(:start_date) { DateTime.parse('2018-01-05 00:00:00')  }
    let(:deadline_date) { DateTime.parse('2018-01-05 00:00:00')  }
    ...
    it 'returns correct date' do
      ...
      expect(project.start_date).to be_a(DateTime)
    end

    it 'returns correct date' do
      expect(project.deadline_date.to_s).to eq(deadline_date.to_s)
      expect(project.deadline_date).to be_a(DateTime)
    end
  end

  context '#to_s' do
    subject(:project) { Project.new 1, 'project1', DateTime.parse('2018-01-05 00:00:00'), DateTime.parse('2018-01-05 00:00:00') }
